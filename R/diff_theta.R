#' Difference between \eqn{\theta}
#'
#' Computes the difference between a starting value of \eqn{\theta} and the \eqn{\theta} estimated with the STF (view details)
#'
#' @param results The object obtained from the stf-generating functions
#' @param true_theta Default is NULL, optional vector of length equal to the number of rows in the original data frame with the true \eqn{\theta} of the respondents. If not available, the \eqn{\theta} estimated with the full-length test will be considered as the starting \eqn{\theta}.
#'
#' @return A data frame with number of rows equal to the number of respondents and 3 columns, one with the starting/true theta, one with the theta estimated with the STF, and the difference between the starting/true and the estimated one.
#' @export
#'
#' @examples
